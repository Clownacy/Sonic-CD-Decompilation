...
void cg_change() { /* Line 119, Address: 0x1026d00 */
  ...
  if (cg_chg1(&tbl_drumb01, 5, &pcgchgtim, &pcgchgcnt, playsubdma, &top) != 0) { /* Line 133, Address: 0x1026d98 */

    for (i = 0; i < 6; ++i, ++top) { /* Line 135, Address: 0x1026dc4 */
...
unsigned int cg_chg1(tile_changes* pTbl, int iNum, unsigned char** ppChgTim, unsigned char** ppChgCnt, int* BmpNo, int* TileStart) { /* Line 162, Address: 0x1026e20 */
  int tblidx, *pNoTbl;
  char Timwk;
  ...
  **ppChgTim = pTbl->dattbl[tblidx].tim; /* Line 181, Address: 0x1026efc */
  ...
  tblidx = pTbl->dattbl[tblidx].no; /* Line 183, Address: 0x1026f18 */
